...
from constants import ZedMini
from laser_stereo_system.util.mathutil import *
from laser_stereo_system.debug.perftimer import PerfTimer
...
def calibrate(data, chessboard_interior_dimensions=(9,6), square_size_m=0.1):
    ...
    reward_timer = PerfTimer()
    gval_timer = PerfTimer()
    subpx_timer = PerfTimer()
    patch_timer = PerfTimer()
    seg_timer = PerfTimer()
    assoc_timer = PerfTimer()
    px_2_3d_timer = PerfTimer()
    ransac_timer = PerfTimer()
